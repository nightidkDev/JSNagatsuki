...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (action == "staff") {
            ...
            new MessageEmbed()
              ...
              .setDescription(
                `Пользователь: ${
                  ...
                  typemod == "chat" ? "Текстовые каналы" : "Голосовые каналы"
      ...
      const age = interaction.components[0].components[0].value;
      const clock = interaction.components[1].components[0].value;
      const expirience = interaction.components[2].components[0].value;
      interaction.editReply({ ephemeral: true, content: "Заявка подана!" });
      interaction.guild.channels.cache
        .get("980467064840003644")
        .send({
          embeds: [
            new MessageEmbed()
              .setTimestamp()
              .setDescription(
                `Пользователь: ${interaction.member}\nВозраст: ${age}\nЧасовой пояс: ${clock}\nОпыт в ивентах: ${expirience}`
              )
              .setColor(COLOR),
          ],
        })
